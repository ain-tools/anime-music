...
export class SavedComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.dataService.isSongPlaying == true) {
      this.title = this.dataService.animeTitle;
  ...
  ngOnDestroy() {
    this.dataService.animeTitle = this.title;
  ...
  getFavoriteMusic() {
    for (let i = 0; i < this.localStorageCounter; i++) {
      let albumOBJ = localStorage.getItem(i + "");
      albumOBJ = JSON.parse(albumOBJ);
      this.favoritesList.push(albumOBJ);
    }
  }
  favoritesList = [];
  albumList = [];
  ...
  currentAnime;
  imgURL;
  getAlbumList(counter) {
    if (this.currentAnime != counter) {
      this.currentAnime = counter;
      this.switch = false;
      this.generateAlbum = true; this.albumList = this.favoritesList[counter].album;
      this.imgURL = this.favoritesList[counter].imgURL;
      this.title = this.favoritesList[counter].title;
      (<HTMLImageElement>document.getElementById("currentAnimePlayingIMG")).src = this.imgURL;
      if (this.albumList.length == 0) {
      ...
      if (document.getElementsByClassName(counter)[0]) {
        document.getElementsByClassName(counter)[0].classList.add("fa-pause");
        document.getElementsByClassName(counter)[0].classList.remove("fa-play");
      }
      ...
      this.pastAnime = counter;
    } else {
      this.stimulateClick(this.currentPlayingSong, this.title);
    }
  ...
  stimulateClick(songName, animeName) {
    if (songName == this.currentPlayingSong && animeName == this.title && !this.switch && songName != undefined ) {
      ...
      this.switch = true;
      this.timeSliderStop();
    } else if (songName == this.currentPlayingSong && animeName == this.title && this.switch && songName != undefined ) {
      //document.getElementById("youtube-icon").click();
      this.dataService.playYtSong = true;
      this.isSongPlaying = true;
      if (document.getElementsByClassName(songName)[0]) {
      ...
      this.playSong(songName, this.title);
      this.isSongPlaying = true;
      if (document.getElementsByClassName(songName)[0]) {
        document.getElementsByClassName(songName)[0].classList.add("fa-pause");
        document.getElementsByClassName(songName)[0].classList.remove("fa-play");
  ...
  incrementAlbumSong() {
    ...
    this.playSong(this.albumList[this.albumListCounter], this.title);
  }
  ...
  randomSongFromAlbum() {
    ...
    this.playSong(this.albumList[randomNumber], this.title);
  }
