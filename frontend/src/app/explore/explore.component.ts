...
export class ExploreComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.dataService.getExploreMusic().subscribe((res) => { this.screenLoader = false;this.newAnime = res.json().newAnime; this.featuredAnime = res.json().featuredAnime; this.popularAnime = res.json().popularAnime; })
    /* setTimeout(() => {
  ...
  getAlbumList(nameClick) {
    if (this.currentAnime != nameClick) {
      (<HTMLImageElement>document.getElementById("currentAnimePlayingIMG")).src = document.getElementsByClassName(nameClick)[0].parentElement.style.backgroundImage.slice(4, -1).replace(/"/g, "");
      this.imgURL = document.getElementsByClassName(nameClick)[0].parentElement.style.backgroundImage.slice(4, -1).replace(/"/g, "");
      ...
      this.dataService.getAlbums(albumObject).subscribe((res) => {
        ...
        if (this.albumList.length == 0) {
          ...
          this.playSong(this.albumList[0], nameClick);
        }
      })
  ...
  playSong(songName, animeName) {
    ...
    this.dataService.getLink(songLinkOBJ).subscribe((res) => {
      ...
      //document.getElementById("youtube-player").setAttribute("src", "https://www.youtube.com/embed/" + this.youtubeLink + "?autoplay=1&loop=1&enablejsapi=1&widgetid=1");
  ...
  loadMoreSongs() {
    let pageRequestOBJ = {
      name: "?limit=" + this.pageNumber,
      action: "loadMoreSongs",
      animeName: "string"
    }
    this.pageNumber = this.pageNumber + 30;
    this.screenLoader = true;
    this.dataService.loadMoreSongs(pageRequestOBJ).subscribe((res) => {
      this.screenLoader = false;
      for (let i = 0; i < res.json().loadedAnime.length; i++) {
        this.loadedAnimeArray.push(res.json().loadedAnime[i]);
      }
    })
  }
